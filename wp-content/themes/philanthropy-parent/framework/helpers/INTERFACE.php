...
if (!function_exists('tfuse_get_suggest')) :
    ...
    function tfuse_get_suggest() {
        ...
        if ($type == 'post') {
            $results = $wpdb->get_results( $wpdb->prepare("SELECT ID, post_title FROM {$wpdb->posts} WHERE post_status = 'publish' AND post_type IN (".$types_sql.") AND post_title LIKE (%s)", '%' . @$wpdb->esc_like($q) . '%') );
            foreach (array_keys($results) as $k) {
                $results[$k] = '<a rel="' . $results[$k]->ID . '">' . $results[$k]->post_title . '</a>';
            }
        } elseif ($type == 'taxonomy') {
            $results = $wpdb->get_results( $wpdb->prepare("SELECT t.name, t.term_id FROM $wpdb->term_taxonomy AS tt INNER JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy IN (".$types_sql.") AND t.name LIKE (%s)", '%' . @$wpdb->esc_like($q) . '%') );
            foreach (array_keys($results) as $k) {
