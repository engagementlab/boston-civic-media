 ...
 */
{
    ...
    function tfuse_get_gallery_images($post_id,$input_id) {
        ...
        $images = array();
        if ($_posts) {
            $query = "SELECT   " . $wpdb->posts . ".* FROM " . $wpdb->posts . "  WHERE 1=1  AND " . $wpdb->posts . ".post_parent = " . $_posts->ID . "  AND " . $wpdb->posts . ".post_type = 'attachment' AND ((" . $wpdb->posts . ".post_status <> 'trash' AND " . $wpdb->posts . ".post_status <> 'auto-draft'))  ORDER BY " . $wpdb->posts . ".post_date DESC ";
